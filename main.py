...
def parens_match_scan(mylist):
    ...
    result = scan(min_f, 0, mapped_list)
    return result[-1] == 0
...
def parens_match_dc_helper(mylist):
    def merge(left, right):
        unmatched_right = max(right[0] - left[1], 0)
        unmatched_left = max(left[0] - right[1], 0)
    ...
    midlength = len(mylist) // 2
    left = mylist[:midlength]
    right = mylist[midlength:]

    l = parens_match_dc_helper(left)
    r = parens_match_dc_helper(right)
